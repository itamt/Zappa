...
For example, if you have a Flask API for ordering a pie, you can call your `bake` function seamlessly in a completely separate Lambda instance by using the `zappa.non_blocking.task` decorator like so:

```python
from flask import Flask
from zappa.non_blocking import task
app = Flask(__name__)
...
When calls to @task decorated functions or the zappa.non_blocking.run command occur outside of Lambda, such as your local dev environment,
...
```python
from zappa.non_blocking import task_sns
@task_sns
```
...
You can also use this functionality without a decorator by passing your function to `zappa.non_blocking.run`, like so:

```python
from zappa.non_blocking import run
...
If those task() parameters were not used, then EC2 would execute the function locally. These same
 `remote_aws_lambda_function_name` and `remote_aws_region` arguments can be used on the zappa.non_blocking.run() function as well.
...
```python
from zappa.non_blocking import task, get_async_response
