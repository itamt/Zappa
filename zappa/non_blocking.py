...
```
   { "command": "zappa.non_blocking.route_lambda_task",
...
5. The new lambda instance is invoked with the message above,
   ...
   dispatches the full message to zappa.non_blocking.route_lambda_task, which
...
class LambdaAsyncResponse(object):
    ...
    def _send(self, message):
        ...
        """
        message['command'] = 'zappa.non_blocking.route_lambda_task'
...
class SnsAsyncResponse(LambdaAsyncResponse):
    ...
    def _send(self, message):
        ...
        """
        message['command'] = 'zappa.non_blocking.route_sns_task'
        payload = json.dumps(message).encode('utf-8')
...
def run(func, args=[], kwargs={}, service='lambda', capture_response=False,
    ...
    import zappa.non_blocking.run
    zappa.non_blocking.run(func, args, kwargs)

    If you want to use SNS, then do:

    zappa.non_blocking.run(func, args, kwargs, service='sns')
